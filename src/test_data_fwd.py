...
def main(run_config=None):
    csv_path = r"data/Zmin2Zmax1_NoDup.csv"
    df_mag = pd.read_csv(csv_path, header=0)
    ...
    params = df_mag.iloc[:, 0:3].values
    # get the columns from 9 to the end
    mag_values = df_mag.iloc[:, 3:].values
...
sweep_id = wandb.sweep(sweep=sweep_configuration, project='metamaterials_forward_test_data_14-4')
